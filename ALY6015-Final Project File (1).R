...
ggplot(data=fifa, aes(x = fifa$Value)) + 
  geom_bar(col = "orange", aes(fill = ..count..)) + ggtitle("Distribution of Value among players")+theme(legend.position = "none")
...
# PCA modelling for fifa dataset by excluding goalkeeper position as the goalkeeper has values = 0. Also removed '0' values in the other columns.
...
# Using the PC1:4 we can explain 95% of the variance. To examine how efficient the model is in comparison, the following linear model is created and evaluated using AIC and BIC.
modeling <- data.frame(fifapca, pcafifa$x[,1:4])
...
yhat1 = predict(decreg2, fifasampletest)
...
library(randomForest)
set.seed(375)
Fifa_RandomForest1<-randomForest(Value~Age+International.Reputation+Overall+Wage+Release.Clause+ Forward + Midfield + Defense + ,data=train_dataset)
...
library(randomForest)
set.seed(445)
Fifa_RandomForest_GK<-randomForest(Value~Age+International.Reputation+Overall+Wage+Release.Clause + Forward + Midfield + Defense + Height+Weight+GKDiving+GKHandling+GKKicking+GKPositioning+GKReflexes, data= train_dataset)
